...
class SinusoidalPosEmb(nn.Module):# 时间位置采样 s() 正弦位置编码
...
class Time_MLP(nn.Module):#时间编码，类似Transformer的位置编码
    def __init__(self, dim):
        super(Time_MLP, self).__init__()
        self.sinusoidaposemb = SinusoidalPosEmb(dim)#对dim做一个正弦编码
...
class Decoder(nn.Module):
    def __init__(self,C_hid, C_out, N_S):
        ...
        self.readout = nn.Conv2d(768, 64, 1)##640
    ...
    def forward(self, hid, enc1=None):
        ...
        ys =Y.shape
        Y = Y.reshape(int(ys[0]/12), int(ys[1]*12), 120, 120)###
        Y = self.readout(Y)
        return Y
...
class IAM4VP(nn.Module):
    def __init__(self, shape_in, hid_S=64, hid_T=512, N_S=4, N_T=6):
        ...
        T, C, H, W = shape_in
        self.time_mlp = Time_MLP(dim=64)#获取时间编码
        self.enc = Encoder(C, hid_S, N_S)#编码器
        self.hid = Predictor(T*hid_S, hid_T, N_T)#预测器
        self.dec = Decoder(hid_S, C, N_S)#解码器
        self.attn = Attention(64)#注意力
        self.readout = nn.Conv2d(64, 1, 1)#1*1卷积
        self.mask_token = nn.Parameter(torch.zeros(12, hid_S, 30, 30))###mask_token可训练
        self.lp = LP(C, hid_S, N_S)#先前学习

    def forward(self, x_raw, y_raw=None, t=None):
        B, T, C, H, W = x_raw.shape
        x = x_raw.view(B*T, C, H, W)# 压缩x的纬度
        time_emb = self.time_mlp(t)# 时间编码
        embed, skip = self.enc(x)# 编码器
        mask_token = self.mask_token.repeat(B,1,1,1,1)#在batch上重复mask

        for idx, pred in enumerate(y_raw):
            embed2,_ = self.lp(pred)#对预测出的pre进行lp
